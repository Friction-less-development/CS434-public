...
all2Instances = np.zeros(shape=(len(TEMPFTESTLIST),4,7))
for i  in range(0, len(TEMPFTESTLIST)):
	for j in range(0, 4):
		tempRow = []
		for k in range(0, 7):
			all2Instances[i][j][k] = TEMPFTESTLIST[i][7*j + k]
...
def getMajority(closestNeighbors):
 ...
 for i in range(0,k):
  if(closestNeighbors[i][0] == 1):
   print "a positive!"
   defaultValue += (len(neg1Instances)+len(neg4Instances)+len(neg6Instances)+len(neg9Instances))
  else:
   defaultValue -= (len(pos1Instances)+len(pos4Instances)+len(pos6Instances)+len(pos9Instances))
 if (defaultValue > 0):
  return 1
 elif(defaultValue < 0):
  return -1
...
def predictTrainingSet(k,f):
 resultY = np.zeros(shape=(len(all2Instances),1))
 for i in range(0,len(all2Instances)):
  print i
  closestNeighbors = np.zeros(shape=(k,2))
  closestNeighbors.fill(-1)
  for j in range(0,len(allInstances)):
   if(True):
    type = hypo[j]
    norm = np.linalg.norm(np.absolute(np.subtract(all2Instances[i][f], transInstances[j][f])))
